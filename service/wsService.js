...
function broadcastToUsers(user, toUsers, webSocketData) {
  toUsers.forEach((e) => {
    if (
      e != user.id &&
      CLIENTS.has(e) &&
      CLIENTS.get(e).readyState === WebSocket.OPEN
    ) {
...
function newUserHandler(stream, user, data) {
  ...
  broadcast(user, new WebSocketData(WebSocketData.Type.NEW_USER, newUser));
}
...
function newConvHandler(stream, user, data) {
  ...
  broadcastToUsers(
    user,
    conv.users,
    new WebSocketData(WebSocketData.Type.NEW_CONV, conv)
  );
}
...
function newMessHandler(stream, user, data) {
  if (convService.addMessage(data)) {
    ...
    broadcastToUsers(
      user,
      conv.users,
      new WebSocketData(WebSocketData.Type.NEW_MESS, data)
    );
  }
}
...
const wsService = {
  ...
  onClose: (stream, user) => {
    ...
    if (offlineUser && offlineUser.online) {
      ...
      broadcast(
        user,
        new WebSocketData(WebSocketData.Type.REM_USER, offlineUser)
      );
    }
  },
};

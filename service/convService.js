...
function shortConvToMeta(convShort) {
  return new ConversationMeta(
    ...
    convShort.messages.length > 0
      ? shortMessageToFull(convShort.messages.at(-1))
      : null
  );
}
...
function shortMessageToFull(message) {
  return new Message(
    userService.getById(
      message.sender?.id ? message.sender.id : message.sender
    ),
    message.date,
    message.text
  );
...
const convService = {
  ...
  addMessage: (message) => {
    ...
    if (conv && sender) {
      if (conv.messages.length >= MAX_MESSAGES_MEM) conv.messages.shift();

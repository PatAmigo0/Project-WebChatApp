export function login(username, onSuccess, onError) {
  fetch("/api/v1/login", {
    method: "POST",
...
export function createConversation(conversation, onSuccess, onError) {
...
export function loadOnlineUsers(onSuccess) {
  loadRequest("/api/v1/users/online", onSuccess, (e) => {});
}

export function loadAllUsers(onSuccess) {
  loadRequest("/api/v1/users", onSuccess, (e) => {});
}

export function loadAllConversations(onSuccess) {
  loadRequest("/api/v1/convs", onSuccess, (e) => {});
}

export function loadConversationById(id, onSuccess, onError) {
...
export class WebSocketConnector {
  ...
  sendNewConversation(id) {
    if (!this.#ws || this.#ws.readyState !== WebSocket.OPEN) {
      console.error(
        "WebSocket не подключен. Невозможно отправить информацию о новой беседе."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "Ошибка при отправке информации о новой беседе через WebSocket:",
        error
      );
    }
  }

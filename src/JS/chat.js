...
export class ChatManager {
  constructor() {
    ...
    this.addChatButton =
      this.chatList.querySelector(".add-chat-button").parentElement;
  ...
  handleLoading() {
    ...
    if (this.users.length == 0) tryLoadAllConversation();
  ...
  handleLoadedConversations(conversations) {
    ...
    this.users = conversations.map((conversation) =>
      this._buildConservation(conversation)
    );
    this.users.sort(
  ...
  async handleReceivedMessage(message) {
    ...
    if (targetChat) {
      ...
      if (
        this.selectedConservationId === message.convId &&
      ...
      tryLoadConversation(message.convId, (conversation) =>
        this.handleNewConservation(conversation, () => {
  ...
  renderUsers() {
    ...
    this.users.forEach((user) => {
      ...
      } else fragment.append(this.chats.get(user.id)); // если чат уже был создан, то просто вставляем его
    });
  ...
  renderMessages(messages) {
    ...
    if (!this.domMesssages.has(this.selectedConservationId)) {
      messages.forEach((message) => {
        ...
        if (!sender) {
          ...
          if (this.currentStreak == 1 || !checkIndentity) {
            ...
            messagesHistoryDiv =
              currentHeader.querySelector(".messages-history");
            ...
            messagesHistoryDiv.append(newFragment);
            if (this.currentStreak == MAX_MESSAGES_IN_ONE_CONTAINER)
              this.currentStreak = 0; // превышен лимит
          } else
            console.error(
              "Этого быть не должно, сообщите Арсению об ошибке :)"
            );
          this.currentStreak++;
        } else {
          this.currentStreak = 1;
      ...
      this.domMesssages.set(
        this.selectedConservationId,
      ...
      this.messagesContainer.innerHTML = this.domMesssages.get(
        this.selectedConservationId
      ); // просто вставляем как innerHtml
    }
  }
  ...
  setupEventListeners() {
    ...
    this.searchBox.addEventListener("input", () =>
      this.searchChats(this.searchBox.value)
    );
  }
  ...
  updateUsersAvatars(value) {
    ...
    this.users.forEach((user) => {
      ...
      else avatarManager.setAvatar(user.id, `${value}/${value}-avatar.png`);

      // на случай если был октрыт чат (нужно изменить аватар пользователя в заголовке чата)
      if (check && this.chats.get(user.id).classList.contains("active")) {
        check = false; // больше не проверять
        this.updateChatHeader(user);
      }
    });

    let imgagesToChange = this.messagesContainer.querySelectorAll(
      "div.message-container"
    );
    imgagesToChange.forEach((userElement) => {
      let userAvatar = userElement.querySelector("img");
      userAvatar.src = avatarManager.getAvatarPath(
        `${userElement.getAttribute("data-sender-id")}`
      );
    });
  }
  ...
  toggleStatus(userId, online) {
    const users = this.chatList.querySelectorAll(
      `[data-companion-id="${userId}"]`
    );
  ...
  sendMessage() {
    ...
    if (messageText) {
      ...
      if (this.selectedConservationId) {
        ...
        console.warn(
          "Не знаю как это произошло, но сообщение отправилось без выбранного чата, ошибка в sendMessage"
        );
    }
  }
  ...
  pushForward() {
    if (
      !(
        this.chatList.firstElementChild.dataset.userId ===
        this.selectedConservationId
      )
    ) {
      const currentEl = this.users.findIndex(
        (us) => us.id === this.selectedConservationId
      );
  ...
  updateChatWindow(user) {
    ...
    if (!messages)
      tryLoadConversation(user.id, (conversation) => {
        ...
        this._onFullConservationLoadSuccess(
      ...
      this._onFullConservationLoadSuccess(
        messages,
        !this.chats.get(user.id).dataset.companionId
      ); // нет компаньена => группа
    }
  }
  ...
  updateChatHeader(user) {
    ...
    avatar.innerHTML = `<img src="${avatarManager.getAvatarPath(
      user.id,
      user.isGroup
    )}" alt="Аватар ${user.isGroup ? "группы" : "пользователя"}">`;
  ...
  async getLastMessage(convId) {
    return new Promise((res, rej) => {
      tryLoadConversation(convId, (conversation) => {
        ...
        else rej("Не получилось получить последнее сообщение");
      });
    });
  }
  ...
  loadCookies() {
    ...
    if (rawChatData) {
      ...
      if (currentToken === chatData.secret_data_token) {
        ...
        if (chatData.chats) this.users = [...chatData.chats];
      } else console.warn("ВЕРСИИ САЙТОВ НЕ СОВПАДАЮТ!");
    }
  }
  ...
  _resizeInput() {
    if (this.messageInput.value.length > 0 && this.enabled == false)
      this._enableChat();
  ...
  _toggleStatus(user, online = true) {
    const fuser = this.users.find(
      (us) => us.companionId === user.dataset.companionId
    ); // пользователь собеседник, я храню его информацию локально чтобы не обращаться к серверу... Поэтому приходится менять ему онлайн ручками
  ...
  async _getLastMessage(conversation) {
    return new Promise((res) => {
      res(
        conversation.messages.length - 1 >= 0
  ...
  _markUnread(convId) {
    ...
    if (chatElement) {
      ...
      this.unreadChats.set(
        convId,
        this.unreadChats.has(convId) ? this.unreadChats.get(convId) + 1 : 1
      );
    }
  }
  ...
  _renderNewMessage(message) {
    ...
    const msg = new Message(message.text, "received", newFragment);
    if (
      this.currentStreak >= MAX_MESSAGES_IN_ONE_CONTAINER + 1 || // для этого костыль снизу
      this.currentStreak == 1 ||
    ...
    if (this.selectedConservationId != null)
      ...
      console.warn(
        "Вообще этого быть не должно, но рендеринг не успел и пользоватеь закрыл чат раньше чем появилось сообщение... Купите новый пк хз (или обновите чат)"
      );
      this.messagesContainer.innerHTML = "";
  ...
  _buildConservation(conversation) {
    const user = {
      ...
      companionId: conversation.users.find((us) => us.id != this.currentUserId)
        .id,
      ...
      online: conversation.users.find(
        (us) => us.online == true && us.id != this.currentUserId
      )
        ? true
        : false,

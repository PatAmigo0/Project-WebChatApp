  ...
  constructor(user, chatList, callback, sibling) {
    ...
    if (!user.isGroup && user.users && user.users.length > 0) {
      ...
      if (currentUserId) {
        ...
        const companion = user.users.find(
          (us) => String(us.id) !== String(currentUserId)
        );
        if (companion) {
    ...
    chatItem.innerHTML = `
            <div class="profile-info"> 
                <div class="avatar">
                    <img src="${avatarManager.getAvatarPath(
                      user.id,
                      user.isGroup
                    )}" alt="Аватар ${
      user.isGroup ? "группы" : "пользователя"
    }">
                </div>
            ...
            <div class="chat-info">
                ...
                <div class="chat-preview">
                    ...
                    ${
                      user.unreadCount > 0
                        ? `<div class="unread-count"><span>${user.unreadCount}</span></div>`
                        : ""
                    }
                </div>
            </div>
    ...
    if (sibling) document.body.insertBefore(chatItem, sibling);
    else chatList.append(chatItem);

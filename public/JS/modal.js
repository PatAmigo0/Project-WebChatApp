import { publicLoadOnlineUsers, tryCreateNewConversation } from "./main.js";
import { modalUser } from "./modalUser.js";

/* Слова */
import { wordsFirst, wordsSecond } from "./words.js";
...
export class ModalWindowHandler {
  ...
  handleUserLeft(userId) {
    ...
    if (this.users.size > 0) this.addButton.classList.add("active");
    else this.addButton.classList.remove("active");
  }
  ...
  handleToggledUser(userId, toggled) {
    if (toggled) this.users.add(userId);
  ...
  setupEventListeners() {
      ...
      tryCreateNewConversation({
        name:
          this.nameInput.value.length > 0
            ? this.nameInput.value.slice(0, 15)
    ...
    // ПОИСК ПОЛЬЗОВАТЕЛЕЙ
    this.searchUsersBox.addEventListener("input", () =>
      this.searchUsers(this.searchUsersBox.value)
    );
  }
  ...
  generateRandomName() {
    ...
    const secondWord =
      wordsSecond[Math.floor(Math.random() * wordsSecond.length)];
